import {View, Text, TextInput, Pressable, Modal} from "react-native";
import {useState, useEffect} from "react";
import MaterialIcons from "react-native-vector-icons/MaterialIcons";
import {DismissKeyboard} from "./dismiss-keyboard";

interface EditTaskModalProps {
  visible: boolean;
  onClose: () => void;
  onUpdateTask: (title: string, description?: string) => Promise<void>;
  initialTitle?: string;
  initialDescription?: string;
  submitting?: boolean;
}

export function EditTaskModal({
  visible,
  onClose,
  onUpdateTask,
  initialTitle = "",
  initialDescription = "",
  submitting = false,
}: EditTaskModalProps) {
  const [title, setTitle] = useState(initialTitle);
  const [description, setDescription] = useState(initialDescription);
  const [error, setError] = useState("");

  useEffect(() => {
    if (visible) {
      setTitle(initialTitle);
      setDescription(initialDescription);
      setError("");
    }
  }, [visible, initialTitle, initialDescription]);

  const handleUpdateTask = async () => {
    if (!title.trim()) {
      setError("Task name is required");
      return;
    }

    try {
      setError("");
      await onUpdateTask(title, description || undefined);
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to update task");
    }
  };

  return (
    <Modal
      visible={visible}
      transparent
      animationType="fade"
      onRequestClose={onClose}
    >
      <DismissKeyboard>
        <View className="items-center justify-center flex-1 bg-black/50">
          <View className="w-11/12 bg-surface-container-high rounded-3xl p-lg gap-lg">
            <View className="flex-row items-center justify-between">
              <Text className="font-headline-sm text-headline-sm text-on-surface">
                Edit Task
              </Text>
              <Pressable onPress={onClose} className="active:opacity-70">
                <MaterialIcons name="close" size={24} color="#A0A7A5" />
              </Pressable>
            </View>

            <View className="gap-sm">
              <Text className="uppercase font-label-md text-label-md text-on-surface-variant">
                Task Name
              </Text>
              <TextInput
                className="w-full border px-md py-lg bg-surface-container-lowest border-outline-variant rounded-xl font-body-md text-body-md text-on-surface"
                placeholder="Enter task name"
                placeholderTextColor="#A0A7A5"
                value={title}
                onChangeText={setTitle}
                editable={!submitting}
              />
            </View>

            <View className="gap-sm">
              <Text className="uppercase font-label-md text-label-md text-on-surface-variant">
                Description
              </Text>
              <TextInput
                className="w-full border px-md py-lg bg-surface-container-lowest border-outline-variant rounded-xl font-body-md text-body-md text-on-surface min-h-24"
                placeholder="Add task description (optional)"
                placeholderTextColor="#A0A7A5"
                value={description}
                onChangeText={setDescription}
                multiline
                editable={!submitting}
              />
            </View>

            {error ? (
              <Text className="font-body-sm text-body-sm text-error">
                {error}
              </Text>
            ) : null}

            <View className="flex-row gap-md">
              <Pressable
                onPress={onClose}
                disabled={submitting}
                className="items-center justify-center flex-1 border rounded-full h-14 bg-surface-container-high border-outline-variant active:opacity-70"
              >
                <Text className="font-label-md text-label-md text-on-surface">
                  Cancel
                </Text>
              </Pressable>

              <Pressable
                onPress={handleUpdateTask}
                disabled={submitting}
                className="items-center justify-center flex-1 rounded-full h-14 bg-primary active:opacity-80"
              >
                <Text className="text-black font-label-md text-label-md">
                  {submitting ? "Saving..." : "Save Changes"}
                </Text>
              </Pressable>
            </View>
          </View>
        </View>
      </DismissKeyboard>
    </Modal>
  );
}
